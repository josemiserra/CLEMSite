...
# Form implementation generated from reading ui file 'C:\Users\JMS\Documents\msite\MSite\msiteSEM\UI\converter.ui'
#
# Created: Tue Jul 26 11:12:04 2016
...
class Ui_Converter(QMainWindow):
    ...
    def setupUi(self, Converter):
        ...
        self.label_prop3 = QLabel(self.groupBox)
        self.label_prop3.setObjectName(_fromUtf8("label_prop3"))
        self.horizontalLayout_9.addWidget(self.label_prop3)
        ...
        self.checkBox_deleteMode = QCheckBox(self.groupBox)
        self.checkBox_deleteMode.setObjectName(_fromUtf8("checkBox_deleteMode"))
        self.checkBox_deleteMode.setText('Delete instead of copy')
        self.gridLayout.addWidget(self.checkBox_deleteMode,9,0,1,1)
        self.label_status = QLabel(self.layoutWidget)
        self.label_status.setObjectName(_fromUtf8("label_status"))
        self.gridLayout.addWidget(self.label_status, 10, 0, 1, 1)
        ...
        self.checkBox_deleteMode.clicked.connect(self.OncheckedDelete)
    ...
    def retranslateUi(self, Converter):
        ...
        self.comboBox_4.setItemText(4, _translate("Converter", "63x", None))
        self.comboBox_4.setItemText(5, _translate("Converter", "--", None))
        self.label_prop3.setText(_translate("Converter", "Zoom factor: ", None))
        ...
        self.comboBox_3.setItemText(14, _translate("Converter", "--", None))
    ...
    def addProfile(self):
        ...
        fluo = self.comboBox_3.currentText()
        if fluo == "--":
            fluo = ""
        objective = self.comboBox_4.currentText()
        if objective == "--":
            objective = ""
        zoomFactor = ""
        if self.spinBox_2.value()> 0 :
            zoomFactor = "--z"+str(self.spinBox_2.value())
        profile_sample["name"] = self.m_common_name +"--"+ fluo + "--" + objective + zoomFactor
    ...
    def run(self):
        ...
        if self.checkBox_deleteMode.isChecked():
            self.applyDelete(flist)
        else:
            dfolder = self.m_destiny_folder
            if not dfolder:
                self.label_status.setText("Status: Please, give a folder where to copy (or delete) the renamed files.")
                return
            self.applyCopy(flist, dfolder)


    def OncheckedDelete(self):
        if self.checkBox_deleteMode.isChecked():
            self.spinBox_2.setEnabled(False)
            self.checkBox_separated.setEnabled(False)
            self.lineEdit_common.setEnabled(False)
            
        else:
            self.spinBox_2.setEnabled(True)
            self.checkBox_separated.setEnabled(True)
            self.lineEdit_common.setEnabled(True)

    def applyDelete(self, flist):
        ...
        for el in flist:
            ...
            self.label_status.setText("Status: All files deleted")

    def applyCopy(self, flist, dfolder):
        items = []
        for index in range(self.listWidget.count()):
            items.append(str(self.profiles[index]["name"]))
        if (not items):
