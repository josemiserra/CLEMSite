...
def Affine_Fit( from_pts, to_pts ):
    ...
    if len(q) != len(p) or len(q)<1:
        raise ValueError("from_pts and to_pts must be of same size.")
    ...
    if not gauss_jordan(M):
        raise FloatingPointError("Error: singular matrix. Points are probably coplanar.")
    ...
    nH = np.concatenate(([np.array(M[0][2:5])],[np.array(M[1][2:5])],[np.array(M[2][2:5])]),axis=0).transpose()
    nH = np.concatenate(([np.array(nH[1])],[np.array(nH[2])],[np.array(nH[0])]),axis=0)        
    return nH
