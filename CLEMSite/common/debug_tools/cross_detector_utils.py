...
import itertools
from common.image_an.common_analysis import canny, ridgeorient
...
def discardwrongpeaks(R,positivePairs,negativePairs,gridsize):
    """  Discard wrong peaks  
    ...
    if(nty<(85) and nty>(60)): print('The angle orientations are not 90 degrees. Adjust properly.')
    return 0,positivePairs,negativePairs
	
def getPeaks(img,img_p,gridsize):
    """
    ...
    error,ppeaks,npeaks = discardwrongpeaks(prj_pos+prj_neg,peaks_pos,peaks_neg,gridsize) # based on the grid definition
    if error<0 :
