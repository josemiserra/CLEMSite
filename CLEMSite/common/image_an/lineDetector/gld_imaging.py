...
def soft_prep(input_image, sigma, clahe=True, canny_thresh=0.05, strokeWidth=20, wiener=[2, 2], laplacian=True,
         compute_inverse_stroke=True, verbose = False):
    """
    ...
    if canny_thresh > 1 or canny_thresh <= 0:
        canny_thresh = 0.05
    if strokeWidth < 2 or strokeWidth > min(w, h):
    ...
    ###############################
    gradient, orientation = canny(PREPRO, 2)
    nm = nonmaxsup_python(gradient, orientation, 1.5)
    print("Gradient and NMS done.")
    BWEDGE = autoedge(nm, canny_thresh, 8, alpha_density=1.1, beta_density=0.0)
    if verbose:
        elapsed = time.time() - t
    ...
    J = borderEnhancer(BWEDGE, wiener)

    _, RELIABILITY = ridgeorient(gradient, 1, 3, 3)
    RELIABILITY[RELIABILITY < 0.5] = 0

    ORIENTIM = calculateOrientations(PREPRO, gradient)
    ORIENTIM[RELIABILITY < 0.5] = 0
    ORIENTIM = np.abs(ORIENTIM)
    tl = np.multiply(J, RELIABILITY)  # Enhance the bw image removing disordered regions

    FSWT = []
    if strokeWidth > 0:
        t = time.time()
        iSWT = SWT(PREPRO, tl, ORIENTIM, strokeWidth, add_invert=compute_inverse_stroke)
        FSWT = cleanswt(iSWT, J)
        elapsed = time.time() - t
        print('Elapsed ' + str(elapsed) + ' seconds.')

    return PREPRO, J, ORIENTIM, RELIABILITY, FSWT
...
def SWT_original(i_img, edgeImage, orientim, stroke_width=20, angle=np.pi / 6):
    ...
    ci, cj = np.nonzero(edgeImage)
    cgrad_x = Ix[ci, cj] * prec
    cgrad_y = Iy[ci, cj] * prec
    ...
    for ind, coord in enumerate(coords):
                ...
                while cres < stroke_width:
                    ...
                    count += 1
                    if (edgeImage[curPixY, curPixX] > 0 and count < stroke_width+1 ):
                        ang_plus_v = ang_plus[i, j]
                        ang_minus_v = ang_minus[i, j]
                        if ((orientim[curPixY, curPixX] < ang_plus_v) and (orientim[curPixY, curPixX] > ang_minus_v) and count > h_stroke):

                            dist = math.sqrt((curPixX - j) * (curPixX - j) + (curPixY - i) * (curPixY - i))
                            for k in range(len(points_x) - 1):
...
def SWT(i_image, edges, orientation, stroke_width, angle=np.pi / 6, add_invert = False):
...
def autoThreshold(nm, canthresh, balance = 0.8):
    """
    DEPRECATED VERSION OF AUTOEDGE
...
def autoedge(nm, canthresh, blocksize, alpha_density = 1.25, beta_density = 0.25):
    ...
    m, n = nm.shape
    patch_size = np.array([m,n]) / blocksize
...
def nonmaxsup_python(gradient,orientation,radius = 1.2):
        ...
        ind_x, ind_y = np.meshgrid(range(rows), range(cols))
