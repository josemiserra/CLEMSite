...
def getInfoHeader(fname):
    ...
    if (second_tag):
        ...
        data['PositionX'] = float(mydict['PositionX']) * 1e3;
        data['PositionY'] = float(mydict['PositionY']) * 1e3;
        data['PositionZ'] = float(mydict['PositionZ']) * 1e3;
        ...
        for page in tif:
            for tag in page.tags.values():
                if (tag.name == '51023' or tag.name =='fibics_xml'):
                    xml_info = tag.value
    if (not xml_info):
        raise FileNotFoundError("NO INFO HEADER for ATLAS picture")
    root = ET.fromstring(xml_info)
    first_tag = [];
    second_tag = [];
    third_tag = [];
