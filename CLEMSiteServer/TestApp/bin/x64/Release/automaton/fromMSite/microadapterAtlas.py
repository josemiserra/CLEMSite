...
import time
import threading
import fromMSite.slugid as slugid
import binascii

from fromMSite.SetupXMLFileManager import *
...
class MicroAdapterAtlas(QObject):
    ...
    def connect(self,isec_key=None):
        """
        ...
        sec_key_g = isec_key
        if(sec_key_g == None):
            sec_key_or = slugid.nice()
            sec_key_g = sec_key_or.hex()
            self.logger.info("Sending key : [" + sec_key_or.decode() + "]\n")
        else:
            if isinstance(sec_key_g, str):
                sec_key_g = sec_key_g.encode('utf-8')
            sec_key_g = sec_key_g.hex()
            self.logger.info("Sending key : [" + str(sec_key_g) + "]\n")
        for el in sec_key_g:
            bval = bin(int(el, 16))[2:].zfill(4)
            a.extend(bval)
   
   #    In the future, add the IP as payload to identify the sender  
   #     ip = self.getNetworkIp().split('.')      
   #     ip_hex = '{:02X}{:02X}{:02X}{:02X}'.format(*map(int, ip))
        ...
        if(connection['Connection']=='Accepted'):
            self.sec_key = connection['Sec-Socket-Accept'];
            self.connected = True
        elif(connection['Connection']=='Denied'):
            self.sec_key = [];
            self.connected = False
            error = self.status
        return  error,self.status,self.sec_key
    ...
    def startAtlas(self):
        ...
        sec_key_g =self.sec_key.encode().hex()
        for el in sec_key_g:  # introducing security key
            bval = bin(int(el, 16))[2:].zfill(4)
            a.extend(bval)
    ...
    def disconnect(self):
        ...
        log =''
        error = ""
        log = "Sending information to close connection to msite4A:\n"
        log = "Connecting with key : "+str(self.sec_key)
    ...
    def initializeSample(self,data_project,workingDir):
        ...
        self.logger.info(" Connecting with key : "+self.sec_key)
        ...
        if(self.connection_is_accepted(header)):
            ...
            payload = payload.hex()
            self.calculatePayload(int(len(payload)*0.5),a)
            en_sec_key =self.sec_key.encode().hex()
            for el in en_sec_key:
                bval = bin(int(el, 16))[2:].zfill(4)
                a.extend(bval)
    ...
    def cancelSample(self):
        ...
        message = ""
        cancel_message = {}
        cancel_message["uri"] ="cancelSample"
        a = bitarray('10000011')
        ...
        try:
            client_socket.connect((self.currentIP, int(self.port)))
            client_socket.sendall(a.tobytes());
            data,header = self.recv_timeout(client_socket,300);
            client_socket.close()
        except client_socket.timeout as msg:
            error = str(msg)
            error = error + "In cancelSample: Connection problems with server."
            return error, str(message)
        except client_socket.error as msg:
            error = str(msg)
            error = error + "In cancelSample: Connection problems with server."
            return error,str(message)
        ...
        log = "Received:"+str(header[3:].decode('utf-8','ignore').replace("\x00",""))+"-."
        if(self.connection_is_accepted(header)):
            message = "Sample cancelled confirmed."
            error = ''
        else:
            error = data
        return  error,str(message)
    ...
    def ping(self):
        ...
        sec_key_g = self.sec_key
        if isinstance(self.sec_key, str):
            sec_key_g = sec_key_g.encode('utf-8')
        sec_key_g = sec_key_g.hex()
        for el in sec_key_g: # introducing security key
            bval = bin(int(el, 16))[2:].zfill(4)
        ...
        if(status == "OK"):
             ...
             if(status == "MESSAGE_READY"):
                ...
                if(message):
                    ...
                    result = re.match('.*PROMPT', str_message)
                    if(result):
                        self.warning.emit(str_message[6:])
                        return
                    result = re.match('.*REQUEST', str_message)
                    if (result):
                        self.request.emit(str_message[result.span()[1]:])
                        return
                    result = re.match('.*CRITICAL', str_message)
                    if(result):
                        self.logger.warning(str_message)
                        self.critical.emit(str_message)
                        return
                    self.new_message.emit(str_message)
    ...
    def setAutoCP(self, master_folder):
        ...
        status_message = {}
        if not self.scale :
            status_message["unit_scale"] = "micrometer"
        else :
        ...
        for el in payload: # introducing security key
            bval = bin(int(el, 16))[2:].zfill(4)
            a.extend(bval)
        self.logger.info('Message sent Automatic Coincidence Point.')
        client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try:
            client_socket.connect((self.currentIP, int(self.port)))
            client_socket.sendall(a.tobytes())
            ...
            error = str(msg)
            error = error + "In autoCP: Connection problems with server."
            return error,""

        message = "Received:"+str(header[3:].decode('utf-8','ignore').replace("\x00",""))
        if(self.connection_is_accepted(header)):
            self.logger.info(message)
            self.logger.info('AutoCP executed.')
            error = ''
        else:
            data = data.decode('utf-8', 'ignore').replace('\x00', '')
            data = data.replace("'", "\"")
