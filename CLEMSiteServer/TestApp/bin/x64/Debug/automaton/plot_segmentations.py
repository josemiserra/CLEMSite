...
im_1 = cv2.imread("test_images\\Focus_Points.tif", cv2.IMREAD_GRAYSCALE)
img,_ = blackBorderDetection(im_1)


print("Felzenszwalb")
start= time.clock()
segments_fz = felzenszwalb(img, scale=300, sigma=2, min_size=(w*h/300))
end= time.clock()
...
print("SLIC")
start= time.clock()
segments_slic = slic(img, n_segments=300, compactness=0.01, sigma=3)
end= time.clock()
ftime= (end-start)/1000
print(ftime)
