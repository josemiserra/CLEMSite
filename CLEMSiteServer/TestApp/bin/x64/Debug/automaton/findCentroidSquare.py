...
def findSquare(file_before,file_after,tag):
    ...
    try:
        ...
        return False, "Square not detected, contour not finded."
    ...
    if len(centroids_list)==0 :
        return False,"No Squares detected. No object found that fits the shape."
    # Closest to center
...
try:
    ...
    tag = sys.argv[3]
    log_txt = "Detecting square. Input parameters: "+file_before+", "+file_after+", "+tag+"."
    valid, error = findSquare(file_before, file_after, tag)
    if (not valid):
        folder_store, file = path.split(file_before)
        log_text = log_text+"\n"+error
        writeErrorFile("ERROR:"+log_txt,folder_store)
except SystemExit:
    pass
except:
